	...
	nrf_gpio_pin_pull_t	pullType, nrf_drv_gpiote_evt_handler_t evt_handler)
{
	...
	switch (input_type)
	{
		...
		{
			in_config =
(nrf_drv_gpiote_in_config_t)GPIOTE_CONFIG_IN_SENSE_LOTOHI(hi_accuracy);
		}
		break;
		case 1:
		{
			in_config =
(nrf_drv_gpiote_in_config_t)GPIOTE_CONFIG_IN_SENSE_HITOLO(hi_accuracy);
		}
		break;
		case 2:
		default:
		{
			in_config =
(nrf_drv_gpiote_in_config_t)GPIOTE_CONFIG_IN_SENSE_TOGGLE(hi_accuracy);
		}
		break;
	}
